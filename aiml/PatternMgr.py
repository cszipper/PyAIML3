...
class PatternMgr:
	...
	def add(self, xxx_todo_changeme, template):
		...
		node = self._root
		for word in pattern.split():#string.split(pattern):
			key = word
			if key == "_":
		...
		if len(that) > 0:
			...
			for word in that.split():#string.split(that):
				key = word
				if key == "_":
					key = self._UNDERSCORE
				elif key == "*":
		...
		if len(topic) > 0:
			...
			for word in topic.split():#string.split(topic):
				key = word
				if key == "_":
					key = self._UNDERSCORE
				elif key == "*":
	...
	def star(self, starType, pattern, that, topic, index):
		...
		# Mutilate the input.  Remove all punctuation and convert the
		# text to all caps.
		input = pattern.upper()#string.upper(pattern)
		input = re.sub(self._puncStripRE, " ", input)
		input = re.sub(self._whitespaceRE, " ", input)
		if that.strip() == "": that = "ULTRABOGUSDUMMYTHAT" # 'that' must never be empty
		thatInput = that.upper()#string.upper(that)
		thatInput = re.sub(self._puncStripRE, " ", thatInput)
		thatInput = re.sub(self._whitespaceRE, " ", thatInput)
		if topic.strip() == "": topic = "ULTRABOGUSDUMMYTOPIC" # 'topic' must never be empty
		topicInput = topic.upper()#string.upper(topic)
		topicInput = re.sub(self._puncStripRE, " ", topicInput)
